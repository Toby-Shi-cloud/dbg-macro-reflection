...
int32_t main() {
  ...
  dbg(message);  // [example.cpp:23 (main)] message = "hello" (std::string)

  const int32_t a = 2;
  const int32_t b = dbg(3 * a) + 1;  // [example.cpp:26 (main)] 3 * a = 6 (int32_t)

  std::vector<int32_t> numbers{b, 13, 42};
  dbg(numbers);  // [example.cpp:29 (main)] numbers = {7, 13, 42} (std::vector<int32_t>)

  dbg("this line is executed");  // [example.cpp:31 (main)] this line is executed

  udf u = {udf::VALUE24, 3.14, {'a', 'b', 'c'}};
  dbg(u);  // [example.cpp:34 (main)] u = {a = VALUE24 (udf_enum), b = 3.14 (double), c = {'a', 'b', 'c'} (vector)} (udf)
